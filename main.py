...
def get_verification_bit(code):
   """

   :param code:кодовое слово длинны 4 для алфавита с равномерным расспределением
   проверочный бит четности добавляется к каждой кодовой букве
   :return: кодовая буква содержащая в конце бит чётности
   """
   if (len(code)==4):
      arr=[0]*4
      ...
      sys.exit("the codeword does not satisfy the length")

def word_coding(word,dict):
   """
   Функия кодировки слова,с проверкой на целостность данных(в случае если равномерное распределение символов)
   :param word:кодируемое слово
   :param dict: словарь соответствия простого символа символу закодированному
   :return:code_word закодированное слово
   """
   code_word=''
   for symbl in word:
      if symbl in dict.keys():

         code_word+=dict[symbl]+get_verification_bit(dict[symbl])
      else:
...
def word_encoding(ensemble_filename,code_word_filename):
   ...
   for i in range(0, len(code_word)):
      arr[i] = code_word[i]
   i = 0
   ind=1#для вывода позиций ошибки
   while (len(arr) != i):
      s = arr[i:i + 4]
      if get_verification_bit(arr[i:i + 4]) == arr[i + 4]:
         print('all good ', arr[i + 4])
         for key in dict_word_code.keys():
            symbol=''.join(arr[i:i + 4])
            if dict_word_code[key]==symbol:
               result+=key

      else:
         result+="?"
         error_message+="\n"+f"error in position {ind}"
   ...
   writing_to_file("errors", error_message)
   return all_data

# word_coding(read_word_from_file("word.txt"),Use_Gilbert_Moore("Prob_0.txt"))

# word_encoding("Prob_0.txt","result.txt")